...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // === NUEVA LÓGICA DE CAPTURA DE CATEGORÍA ===
    if (esPaginaTienda) {
        const urlParams = new URLSearchParams(window.location.search);
        const catFiltro = urlParams.get('categoria');

        if (catFiltro) {
            // Buscamos el checkbox que coincide (convertimos a minúsculas para evitar fallos)
            const checkbox = document.querySelector(`.filter-check[value="${catFiltro.toLowerCase()}"]`);
            if (checkbox) {
                checkbox.checked = true;
                // Abrimos visualmente el acordeón de categorías (opcional)
                const opts = document.getElementById('aroma-opts');
                if(opts) opts.style.display = 'block';
                
                // Ejecutamos el filtrado DESPUÉS de que Firebase cargue los productos
                // Usamos un pequeño delay o lo llamamos dentro del observador de Firebase
            }
        }
    }
...
window.verificarAcceso = function() {
    ...
    if (adminTools) {
        ...
        adminTools.innerHTML = `
    <div class="auth-container">
        <a href="admin.html" class="btn-nav-auth admin-highlight"><i class="fas fa-tools"></i> ADMIN</a>
...
function cargarProductosTienda() {
    ...
    db.ref('productos').on('value', (snapshot) => {
        ...
        listaProductos.forEach(p => {
            // 1. Lógica de Precios y Ofertas
            const precioActual = parseFloat(p.precio) || 0;
            const precioViejo = parseFloat(p.precioOferta) || 0;
            const esOferta = precioViejo > precioActual; // Si el original es mayor al actual, hay oferta

            // 2. Limpieza de categoría para el filtro
            const catLimpia = (p.aroma || p.categoria || 'otros').toLowerCase().trim();

const card = `
    <div class="product-card ${p.isAgotado ? 'card-agotada' : ''}" 
         data-category="${catLimpia}" 
         data-price="${precioActual}"
         data-is-oferta="${esOferta}">
        ...
        ${esAdmin ? `
            <div class="admin-card-header">
                <div class="admin-btn-group">
                    <button class="btn-admin-edit" onclick="irAEditar('${p.id}')"><i class="fas fa-pencil-alt"></i></button>
                    <button class="btn-admin-delete" onclick="borrarDesdeTiendaDirecto('${p.id}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>` : ''}
        ...
        <div class="product-info">
            ...
            <div class="price-container">
                ${esOferta ? `<span class="old-price">$ ${precioViejo.toFixed(2)}</span>` : ''}
                <span class="current-price">$ ${precioActual.toFixed(2)}</span>
            </div>
            ...
            ${!p.isAgotado ? `
                ...
                <button class="add-to-cart-btn btn-disabled" disabled>AGOTADO</button>
            `}
        </div>
    </div>`;
        ...
        // === EL FIX DE VISIBILIDAD ===
        const urlParams = new URLSearchParams(window.location.search);
        const tieneFiltro = urlParams.get('categoria') || urlParams.get('cat');
...
function filtrarProductos() {
    ...
    const maxP = parseFloat(document.getElementById('priceRange')?.value) || 1000;
    ...
    if (categoriasSeleccionadas.length > 0) {
        url.searchParams.set('categoria', categoriasSeleccionadas[0]);
    } else {
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        const esOfertaCard = card.getAttribute('data-is-oferta') === 'true';

        // LÓGICA DE VALIDACIÓN
        const cumpleNombre = nombre.includes(busqueda);
        const cumplePrecio = precio <= maxP;
        
        // Validación de categoría (Soporta la categoría "ofertas")
        const cumpleCategoria = categoriasSeleccionadas.length === 0 || categoriasSeleccionadas.some(cat => {
            if (cat === 'ofertas') {
                return esOfertaCard || categoriaCard === 'ofertas';
            }
            return cat === categoriaCard;
        });
...
window.toggleFilter = function(id) {
    ...
    span.innerText = opts.style.display === 'block' ? '↗︎' : '↙︎';
};

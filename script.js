...
function filtrarProductos() {
    ...
    // === NUEVA LÓGICA: Sincronizar con la URL ===
    const url = new URL(window.location);
    if (categoriasSeleccionadas.length > 0) {
        // Guardamos la primera categoría para que la URL sea limpia: ?categoria=perfumes
        url.searchParams.set('categoria', categoriasSeleccionadas[0]);
    } else {
        url.searchParams.delete('categoria');
    }
    // Actualiza la barra de direcciones sin recargar la página
